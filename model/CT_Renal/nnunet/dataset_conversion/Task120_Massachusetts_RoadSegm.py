...
from model.CT_Renal.nnunet.utilities.file_conversions import convert_2d_image_to_nifti
...
if __name__ == '__main__':
    ...
    Working with 2D data in nnU-Net requires a small workaround in the creation of the model. Essentially, all images 
    ...
    'regular' 2D images. We selected the massachusetts road segmentation model for this because it can be obtained 
    easily, it comes with a good amount of training cases but is still not too large to be difficult to handle.
    """

    # download model from https://www.kaggle.com/insaff/massachusetts-roads-dataset
    # extract the zip file, then set the following path according to your system:
    base = '/media/alex/linux-data/shuju'
    ...
    # finally we can call the utility for generating a model.json
    generate_dataset_json(join(target_base, 'model.json'), target_imagesTr, target_imagesTs, ('Red', 'Green', 'Blue'),
                          labels={1: 'street'}, dataset_name=task_name, license='hands off!')

    """
    once this is completed, you can use the model like any other nnU-Net model. Note that since this is a 2D
    model there is no need to run preprocessing for 3D U-Nets. You should therefore run the 
    ...
    a different 2D model, you can make nnU-Net determine the postprocessing by using the
