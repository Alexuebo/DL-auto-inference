...
def load_dataset(folder, num_cases_properties_loading_threshold=1000):
    ...
    for c in case_identifiers:
        ...
        # model[c]['properties'] = load_pickle(join(folder, "%s.pkl" % c))
...
class DataLoader3D(SlimDataLoaderBase):
    def __init__(self, data, patch_size, final_patch_size, batch_size, has_prev_stage=False,
        ...
        Why all the hassle? Well the decathlon model is huge. Using npy for everything will consume >1 TB and that is uncool
        ...
        :param random: Sample keys randomly; CAREFUL! non-random sampling requires batch_size=1, otherwise you will iterate batch_size times over the model
...
class DataLoader2D(SlimDataLoaderBase):
    def __init__(self, data, patch_size, final_patch_size, batch_size, oversample_foreground_percent=0.0,
        ...
        :param random: sample randomly; CAREFUL! non-random sampling requires batch_size=1, otherwise you will iterate batch_size times over the model
