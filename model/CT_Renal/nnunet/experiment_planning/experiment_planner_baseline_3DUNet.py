...
from model.CT_Renal.nnunet import DatasetAnalyzer
...
from model.CT_Renal.nnunet.network_architecture import Generic_UNet
...
class ExperimentPlanner(object):
    ...
    def get_properties_for_stage(self, current_spacing, original_spacing, original_shape, num_cases,
        ...
        """
        Computation of input patch size starts out with the new median shape (in voxels) of a model. This is
        ...
        # clip it to the median shape of the model because patches larger then that make not much sense
    ...
    def plan_experiment(self):
        ...
        print("the median shape of the model is ", median_shape)

        max_shape = np.max(np.vstack(new_shapes), 0)
        print("the max shape in the model is ", max_shape)
        min_shape = np.min(np.vstack(new_shapes), 0)
        print("the min shape in the model is ", min_shape)
        ...
        print("the transposed median shape of the model is ", median_shape_transposed)
...
if __name__ == "__main__":
    ...
    for t in tasks:
        try:
            ...
            shutil.copy(join(nnUNet_raw_data, t, "model.json"), preprocessing_output_dir_this_task)
