...
def split_4d(input_folder, num_processes=default_num_threads, overwrite_task_output_id=None):
    ...
    p.close()
    p.join()
    shutil.copy(join(input_folder, "model.json"), output_folder)


def create_lists_from_splitted_dataset(base_folder_splitted):
    lists = []

    json_file = join(base_folder_splitted, "model.json")
...
def crop(task_string, override=False, num_threads=default_num_threads):
    ...
    shutil.copy(join(nnUNet_raw_data, task_string, "model.json"), cropped_out_dir)
...
def plan_and_preprocess(task_string, processes_lowres=default_num_threads, processes_fullres=3, no_preprocessing=False):
    ...
    from model.CT_Renal.nnunet.experiment_planning.experiment_planner_baseline_3DUNet import ExperimentPlanner
    ...
    shutil.copy(join(nnUNet_raw_data, task_string, "model.json"), preprocessing_output_dir_this_task_train)
