...
from model.CT_Renal.nnunet.configuration import default_num_threads
...
def verify_dataset_integrity(folder):
    """
    folder needs the imagesTr, imagesTs and labelsTr subfolders. There also needs to be a model.json
    ...
    :return:
    """
    assert isfile(join(folder, "model.json")), "There needs to be a model.json file in folder, folder=%s" % folder
    ...
    dataset = load_json(join(folder, "model.json"))
    ...
    # check all cases
    if len(expected_train_identifiers) != len(np.unique(expected_train_identifiers)): raise RuntimeError("found duplicate training cases in model.json")
    ...
    for c in expected_train_identifiers:
        ...
        expected_image_files = [join(folder, "imagesTr", c + ".nii.gz") for i in range(num_modalities)]
    ...
    assert len(
        nii_files_in_imagesTr) == 0, "there are training cases in imagesTr that are not listed in model.json: %s" % nii_files_in_imagesTr
    assert len(
        nii_files_in_labelsTr) == 0, "there are training cases in labelsTr that are not listed in model.json: %s" % nii_files_in_labelsTr
    ...
    if fail:
        raise AssertionError(
            "Found unexpected labels in the training model. Please correct that or adjust your model.json accordingly")
    else:
        print("Labels OK")
    ...
    if len(expected_test_identifiers) > 0:
        ...
        for c in expected_test_identifiers:
            ...
            expected_image_files = [join(folder, "imagesTs", c + ".nii.gz") for i in range(num_modalities)]
            print(expected_image_files)
            assert all([isfile(i) for i in
                        expected_image_files]), "some image files are missing for case %s. Expected files:\n %s" % (
                c, expected_image_files)
        ...
        assert len(
            nii_files_in_imagesTs) == 0, "there are training cases in imagesTs that are not listed in model.json: %s" % nii_files_in_imagesTr

    all_same, unique_orientations = verify_all_same_orientation(join(folder, "imagesTr"))
    if not all_same:
        print(
            "WARNING: Not all images in the model have the same axis ordering. We very strongly recommend you correct that by reorienting the data. fslreorient2std should do the trick")
    # save unique orientations to model.json
